...
Func _Get_ImagePath()
	...
	_FileWriteLog($sLogPath, "------4.检查镜像文件*开始------")
	...
	Else
		...
		For $i = 1 To $aDriveArray[0]
			...
			If FileExists($sDownloadImagePath) Then
				_FileWriteLog($sLogPath, "成功;检测到目标镜像")
				ExitLoop
			Else
		...
		_FileWriteLog($sLogPath, "失败;没有检测到目标镜像：" & $sDownloadImagePath & "，请联系镜像维护人员")
				FileCopy($sLogPath, $sServerLogPath, $FC_OVERWRITE)
				DirCopy($sLogDirPath, $sServerLogDirPath, $FC_OVERWRITE)
				Exit
		EndIf
	...
	_FileWriteLog($sLogPath, "------4.检查镜像文件*完成------")
	_FileWriteLog($sLogPath, "==============================================================================================")
	FileCopy($sLogPath, $sServerLogPath, $FC_OVERWRITE)
